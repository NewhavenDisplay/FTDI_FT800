...
void interruptService(){
	...
	/*Read register REG_INT_FLAGS, each bit of the register corresponding to an interrupt source, the register is clear on read.*/
	interruptFlags = FTImpl.Read(REG_INT_FLAGS);
	
	/*Check the interrupt sources to see if it's the TAG source.  The TAG interrupt will occur two times because it's an interrupt for the change of tag value, the first time is for the initial touch of an tagged object and the second time is after an tagged object was no longer being touched.*/ 
