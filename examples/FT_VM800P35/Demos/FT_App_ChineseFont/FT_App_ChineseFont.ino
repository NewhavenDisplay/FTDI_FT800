...
* @brief                          Sample application to demonstrate FT800 custom fonts. 
* @version                        0.1.0
* @date                           2014/03/21
*
*/
...
void readfile2flash(const char *filename, uint32_t FT800_addr)
{
FT_SDStatus Status;
  FT_SDFile fontfile;
  byte file = 0; 
  ...
  if(Status){
	...
	while (fontfile.Offset < fontfile.Size)
	{
		...
		//FTImpl.WriteCmd(FT800_addr,imbuff,n);  //need replacement function
		FTImpl.Write(FT800_addr,imbuff,n);
		FT800_addr += n;
	}
  }
}
