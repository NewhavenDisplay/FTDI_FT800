...
/**
* @file                           PowerModes.ino
* @brief                          Sketch to demonstrate various power modes supported in FT800.
...
/* This application demonstrates the usage of FT800 library on Breakout_4DLCD_FT843 platform */
...
/* API to demonstrate various power modes of FT800. Also display string on the screen for informative purposes */
void PowerModes()
{
	/*************************************************
	...
	**************************************************/
	...
	fadeout();
	FTImpl.PlaySound(0,FT_MUTE);//play mute sound - feature in FT800 to avoid audio glitch during power down stage. also note that audio engine plays unmute sound by default after hardware reset
	delay(25);
	FTImpl.HostCommand(FT_STANDBY);//send standby command to FT800 - power to PLL and crystal is enabled, rest all cut off, registers contents are retained. Instantaneous wakeup, 1.5mA current consumed

	//Wake up from Standby first before accessing FT800 registers.
	FTImpl.HostCommand(FT_ACTIVE);//send active command to FT800
	...
	**************************************************/
    //Switch FT800 from Active to delay mode
