...
*****************************************************************************/
/**
* @file                           FT_App_Sketch.ino
* @brief                          This file demonstrates the usage of interactive sketch function.
...
//  Set the bitmap properties , sketch properties and Tracker for the sliders
  FTImpl.DLStart();
  FTImpl.Cmd_FGColor(0xffffff);        // Set the bg color
  FTImpl.Cmd_Track(FT_DISPLAYWIDTH-30,40,8,FT_DISPLAYHEIGHT-100,1);
  FTImpl.Cmd_Sketch(0,10,FT_DISPLAYWIDTH-40,FT_DISPLAYHEIGHT-30,0,FT_L8);
  FTImpl.Cmd_Memzero(0L,256*1024L);  
  FTImpl.BitmapSource(0);
  FTImpl.BitmapLayout(FT_L8,FT_DISPLAYWIDTH-40,FT_DISPLAYHEIGHT-20);
  FTImpl.BitmapSize(FT_NEAREST,FT_BORDER,FT_BORDER,(FT_DISPLAYWIDTH -40),(FT_DISPLAYHEIGHT-20));   
  FTImpl.Cmd_Swap();
  FTImpl.Finish();				
  while(1)
  {
    ...
    {
		FTImpl.DLStart( );  
		FTImpl.Cmd_Memzero(0,256*1024L); 	// Clear the gram frm 1024 		
		FTImpl.Finish();		
    }
    ...
    FTImpl.Clear(1,1,1);	  				// clear the display     
    FTImpl.ColorRGB(255,255,255);  			// color	
    FTImpl.Cmd_BGColor(color);   
    FTImpl.TagMask(1);						 	
    FTImpl.Tag(1);         					 // assign the tag value 
    FTImpl.Cmd_FGColor(color);
   // draw the sliders 
    FTImpl.Cmd_Slider(FT_DISPLAYWIDTH -30,40,8,(FT_DISPLAYHEIGHT-100),0,val,65535);	 
    FTImpl.Cmd_FGColor(tag==2?0x0000ff:color);
    FTImpl.Tag(2);          					
    FTImpl.Cmd_Button(FT_DISPLAYWIDTH -35,(FT_DISPLAYHEIGHT-45),35,25,26,0,"CLR");
    FTImpl.TagMask(0); 
    FTImpl.Cmd_Text(FT_DISPLAYWIDTH -35,10,26,0,"Color"); 
    FTImpl.LineWidth(1*16);
    FTImpl.Begin(FT_RECTS);
    FTImpl.Vertex2f(0,10*16);
    FTImpl.Vertex2f((int16_t)(FT_DISPLAYWIDTH -40)*16,(int16_t)(FT_DISPLAYHEIGHT-20)*16);			
    FTImpl.ColorRGB(color);
    FTImpl.Begin(FT_BITMAPS);
    FTImpl.Vertex2ii(0,10,0,0);
    FTImpl.DLEnd(); //end the display list
	FTImpl.Finish(); //render the display list and wait for the completion of the DL	
  }
}
